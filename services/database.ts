...
export const DatabaseService = {
  ...
  saveDocument: async (doc: SchoolDocument) => {
    ...
    if (doc.id && doc.id.length > 10) {
        await supabase.from('documents').update(dbDoc).eq('id', doc.id);
    } else {
  ...
  saveStaff: async (staff: StaffMember) => {
    ...
    if (staff.id && staff.id.length > 10) {
        await supabase.from('staff_members').update(dbStaff).eq('id', staff.id);
    } else {
        await supabase.from('staff_members').insert(dbStaff);
    }
  },
